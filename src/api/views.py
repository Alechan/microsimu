...
class RegionResultDetail(APIView):
    ...
    You can find visualization examples for these results here: <{visualize_url}>
    """
    renderer_classes = APIView.renderer_classes
    ...
    def description(self):
        ...
        """
        base_description = self.__class__.__doc__
        visualize_kwargs = self.request.parser_context["kwargs"]
        description = base_description.format(
            visualize_url=self.request.build_absolute_uri(reverse("api:visualize", kwargs=visualize_kwargs)),
        )
        return description
